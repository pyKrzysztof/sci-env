...
class ButtonBase(wx.Control):
    ...
    def __init__(self, parent, config, label, **kwargs):
        wx.Control.__init__(self, parent, id=wx.ID_ANY, style=wx.NO_BORDER, **kwargs)

        self.label = label
        self.config = config

        self.font = config.Get('regFont', True)
        if label == '☰':
            print('label is ☰')
...
class PanelButton(wx.Control):
    ...
    def Draw(self, dc):
        ...
        if self.isActive:
            ...
            dc.SetBackground(wx.Brush(self.conf.Get('inactiveColor')))  
        dc.Clear()
...
class MenuButton(ButtonBase):
    ...
    def __init__(self, parent, config, **kwargs):
        ButtonBase.__init__(self, parent=parent, config=config, label='☰', **kwargs)
    ...
    def Draw(self, dc):
        ...
        dc.SetFont(self.font)
        dc.SetTextForeground(self.config.Get('textColor'))
        dc.SetBackground(wx.Brush(self.config.Get('menuColor')))
        dc.Clear()
...
class MenuPanelSwitchButton(ButtonBase):
    ...
    def Draw(self, dc):
        ...
        dc.SetFont(self.config.Get('menuButtonFontBig', True))
        dc.SetTextForeground(self.config.Get('textColor'))
        dc.SetBackground(wx.Brush(self.config.Get('buttonColor')))
        if self._mouseIn:
            dc.SetBackground(wx.Brush(self.config.Get('highlightColor')))
        ...
        dc.SetPen(wx.Pen(self.config.Get('borderColor'), width=5))
        dc.SetBrush(wx.TRANSPARENT_BRUSH)
        dc.DrawRectangle(0, 0, width, height)
...
class ConfigColorSwitcher(wx.Control):
    ...
    def __init__(self, parent, label, color, conf, size, **kwargs):
        wx.Control.__init__(self, parent, id=wx.ID_ANY, size=size, style=wx.NO_BORDER, **kwargs)
...
class CheckBox(wx.Control):
    ...
    def Draw(self, dc):
        ...
        if self.label.find('//') != -1:
            ...
            newWidth = (text1Width if text1Width > text2Width else text2Width) + newHeight
            ...
            width, height = self.GetClientSize()
            textX, textY = (width-height-textWidth) / 2, (height-textHeight) / 2
            dc.DrawText(self.label, textX, textY)

        dc.SetPen(wx.Pen(self.conf.Get('borderColor'), width=2))
        dc.SetBrush(wx.Brush(self.conf.Get('selectColor') if self.value else self.conf.Get('inactiveColor')))
        padding = (height-self.rectSize) / 2
        dc.DrawRectangle((width-height+padding), padding, self.rectSize, self.rectSize)