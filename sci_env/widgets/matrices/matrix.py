...
class Matrix(wx.Panel):
	...
	def DisplayGrid(self, rows, cols):
		...
		self.panel.SetBackgroundColour(self.appConf.Get('widgetColor1'))
		self.panel.sizer = wx.GridSizer(rows, cols, 0, 0)

		size = (self.fontSize+20, self.fontSize+5)
		fg = self.appConf.Get('widgetTextColor')
		bg = self.appConf.Get('widgetColor1')
	...
	def Draw(self, dc):
		...
		dc.SetPen(wx.Pen(self.appConf.Get('widgetColor2'), width=5))
		dc.DrawRectangle(0, 0, width, height)
		padding = self.fontSize
		dc.SetPen(wx.Pen(self.appConf.Get('widgetColor1'), width=10))
...
class MainPanel(wx.Panel):
	...
	def Display(self):
		...
		textFont = self.appConf.Get('regFont', True)
		textFont.SetPointSize(20)

		dim_a = TextField(self, textFont, '#ffffff', '#32404b', size=(200, 30), name='a')
		dim_a.Bind(wx.EVT_TEXT_ENTER, self.ChangeDimensions)
		dim_a.SetValue('3x3')

		dim_b = TextField(self, textFont, '#ffffff', '#32404b', size=(200, 30), name='b')
		dim_b.Bind(wx.EVT_TEXT_ENTER, self.ChangeDimensions)
		dim_b.SetValue('3x3')

		x_text = ScaledStaticText(self, 'x', textFont, 
								  self.appConf.Get('widgetColor1'), self.appConf.Get('widgetTextColor'),
								  size=(20, 20))

		self.matrix_a = Matrix(self, self.appConf, matrixFont, dimensions='3x3')
		self.matrix_b = Matrix(self, self.appConf, matrixFont, dimensions='3x3')
		self.matrix_c = Matrix(self, self.appConf, matrixFont, dimensions='3x3')
